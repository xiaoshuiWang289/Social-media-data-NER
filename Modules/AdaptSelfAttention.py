...
class AdaptSelfAttention(nn.Module):
    def __init__(self, hidden_size, num_heads,
        ...
        if dataset == 'weibo':
            self.randomAttention = nn.Parameter(torch.empty(1, self.num_heads, 512, 512), requires_grad=True)
        if dataset == 'msra':
            self.randomAttention = nn.Parameter(torch.empty(1, self.num_heads, 310, 310), requires_grad=True)
        if dataset == 'resume':
            self.randomAttention = nn.Parameter(torch.empty(1, self.num_heads, 570, 570), requires_grad=True)
    ...
    def forward(self, query, key, value, seq_len, lex_num, rel_pos_embedding):
        ...
        print('batch',batch,'max_seq_len',max_seq_len)
